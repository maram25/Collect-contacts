...
public class GroupsAdapter extends RecyclerView.Adapter<GroupsAdapter.ViewHolder> {
	...
	public void onBindViewHolder(@NonNull ViewHolder holder, @SuppressLint("RecyclerView") int position) {
		...
		if (Utils.checkList) {
			...
			for (int i = 0; i < Groups.get(position).getPhones().size(); i++) {

				Utils.GroupsPhones.add(Groups.get(position).getPhones().get(i));
			}
		...
		holder.check_group_item.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
				 if(holder.check_group_item.isChecked()){
					 for (int i = 0; i < Groups.get(position).getPhones().size(); i++) {
						 Utils.selected_itemsGroupsPhones.add(Groups.get(position).getPhones().get(i));
					 }
				 }
				  else
				 {
					 for (int i = 0; i < Groups.get(position).getPhones().size(); i++) {
						 Utils.selected_itemsGroupsPhones.remove(Groups.get(position).getPhones().get(i));
						 Utils.GroupsPhones.remove(Groups.get(position).getPhones().get(i));
					 }
